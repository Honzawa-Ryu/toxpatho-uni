...
**方針：C を本命に置くなら A を必ず並走させる。** A は「そのタスクでの到達上限」を与え、
...
1. 手持ちデータの規模（スライド枚数・施設数・スキャナ種）— 外部検証として成立する規模か。
2. **ROI アノテーションを少量作る余地があるか。** あれば選択肢 A が開け、主張が大幅に堅くなる。
3. 継続 SSL にこだわる理由は「新規性の確保」か「アノテーションコストの回避」か。
   - 前者 → 用量反応単調性を評価軸として前面に出す構成
   - 後者 → 選択肢 A への切り替えを検討
